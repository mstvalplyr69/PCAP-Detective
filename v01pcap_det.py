...
def create_tagged_packet(target_ip, packet_size, tag_text):
    """
    Create a packet with a loud identifier and a specific length.
    """
    # Create an IP layer with the destination as the target IP
    ip = scapy.IP(dst=target_ip)
    
    # Create a UDP layer with random source and destination ports
    udp = scapy.UDP(sport=12345, dport=12345)
    
    # Create the payload with the tag and pad it to the desired length
    payload = tag_text.ljust(packet_size, 'X')  # Padding with 'X' if needed
...
def run_pcap_detective():
    ...
    tagged_packet = create_tagged_packet(args.target_ip, args.packet_size, args.tag_text)
